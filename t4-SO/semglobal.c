...
int selecionaPosicao(Lugar *l, int ind, int tam);
int transacao(void);
int semCompradores(void);
void situacaoFinal(Lugar *l, int tam);
...
int main(int argc, char *argv[]){
	...
	parCompradora->lugares = malloc(sizeof(Lugar) * atoi(argv[1]));
    preencheVetorDeLugares(parCompradora->lugares, atoi(argv[1]));
    ...
    //situacaoFinal(parCompradora->lugares, atoi(argv[1]));

    free(parCompradora->lugares);
    sem_destroy(&mutex);
...
int selecionaPosicao(Lugar *l, int ind, int tam){
	...
	printf("AQUI5\n");
	l[flag].estado = -1; //impossibilita a seleção do mesmo local
	printf("AQUI6\n");
...
void *compraIngresso(void* arg){
	...
	pos = selecionaPosicao(p.lugares,p.ind, p.nIngressos); 
	printf("AQUI4\n");

	printf("AQUIII %d\n", p.lugares[pos].estado);

	if(transacao()){
		printf("AQUI2\n");
		p.lugares[pos].estado = 0;
		printf("AQUI\n");
		p.lugares[pos].dono = p.ind;
		printf("AQUI3\n");
		/*lugares[pos].estado = 0;
		lugares[pos].dono = p.ind;*/
		printf("Thread %d conseguiu comprar o lugar %d\n", p.ind, pos + 1);
	}else{
		p.lugares[pos].estado = 1;
